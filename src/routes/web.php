...
Route::middleware(['web', \App\Http\Middleware\LanguageMiddleware::class])
    ...
    Route::middleware('auth:user')->prefix($routes['user'])->group(function () use ($routes) {
        ...
        Route::patch($routes['notifications'] . '/{id}/read', [UserNotificationController::class, 'markAsRead'])->name('user.notifications.read');
    ...
    Route::middleware('auth:admin')->prefix($routes['admin'])->group(function () use ($routes) {
        ...
        Route::prefix('tickets')->group(function () use ($routes) {
            ...
            Route::patch('/{ticket}/' . $routes['update_status'], [AdminTicketController::class, 'updateTicketStatus'])->name('admin.update.ticket');
            Route::patch('/{ticket}/' . $routes['close'], [TicketController::class, 'closeTicket'])->name('admin.close.ticket');
            Route::post('/{ticket}/' . $routes['reopen'], [TicketController::class, 'reopenTicket'])->name('admin.reopen.ticket');
            Route::post('/{ticket}/' . $routes['assign'], [AdminTicketController::class, 'assignTicket'])->name('admin.assign.ticket');
        });

        Route::prefix($routes['comments'])->group(function () use ($routes) {
            Route::post('/{ticket}/' . $routes['comment'], [CommentController::class, 'addComment'])->name('admin.add.comment');
            Route::delete('/{comment}', [CommentController::class, 'deleteComment'])->name('admin.delete.comment');
            Route::get('/{ticket}/' . $routes['comments'], [CommentController::class, 'viewComments'])->name('admin.view.comments');
        });

        Route::prefix($routes['users'])->group(function () use ($routes) {
            Route::get('/', [AdminUserController::class, 'showListUsers'])->name('admin.dashboard.list.users');
            Route::get($routes['dashboard'], [AdminUserController::class, 'showAddDashboard'])->name('admin.dashboard.add');
            Route::get($routes['create'], [AdminUserController::class, 'createUser'])->name('admin.users.create');
            Route::post('/', [AdminUserController::class, 'storeUser'])->name('admin.users.store');
            Route::get('/{user}/' . $routes['edit'], [AdminUserController::class, 'editUser'])->name('admin.users.edit');
            Route::put('/{user}', [AdminUserController::class, 'updateUser'])->name('admin.users.update');
            Route::get('/{user}/' . $routes['confirm_delete'], [AdminUserController::class, 'confirmDeleteUser'])->name('admin.users.confirmDelete');
            Route::delete('/{user}', [AdminUserController::class, 'confirmDeleteUserPost'])->name('admin.users.destroy');    
        });



        Route::prefix($routes['admins'])->group(function () use ($routes) {
            Route::get($routes['list'],[AdminAdminController::class, 'showListAdmins'])->name('admin.dashboard.list.admins');
            Route::get($routes['create'],[AdminAdminController::class, 'createAdmin'])->name('admin.admins.create');
            Route::post($routes['admins'],[AdminAdminController::class, 'storeAdmin'])->name('admin.admins.store');
            Route::get('/{admin}/' . $routes['edit'],[AdminAdminController::class, 'editAdmin'])->name('admin.admins.edit');
            Route::put('/{admin}',[AdminAdminController::class, 'updateAdmin'])->name('admin.admins.update');
            Route::get('/{admin}/' . $routes['confirm_delete'],[AdminAdminController::class, 'confirmDeleteAdmin'])->name('admin.admins.confirmDelete');
            Route::delete('/{admin}',[AdminAdminController::class, 'confirmDeleteAdminPost'])->name('admin.admins.destroy');
        });



        Route::prefix($routes['types'])->group(function () use ($routes) {
            Route::get('/', [TypesController::class, 'index'])->name('admin.types.index');
            Route::get('/' . $routes['create'], [TypesController::class, 'create'])->name('admin.types.create');
            Route::post('/', [TypesController::class, 'store'])->name('admin.types.store');
            Route::get('/{type}/' . $routes['edit'], [TypesController::class, 'edit'])->name('admin.types.edit');
            ...
            Route::get('/{type}/' . $routes['confirm_delete'], [TypesController::class, 'confirmDelete'])->name('admin.types.confirmDelete');
        });

        Route::prefix($routes['notifications'])->group(function () use ($routes) {
            Route::get('/', [AdminNotificationController::class, 'showNotifications'])->name('admin.notifications');
            Route::patch('/{notificationId}/' . $routes['read'], [AdminNotificationController::class, 'markAsRead'])->name('admin.notifications.read');
        });

        Route::get('/admin/history/events', [EventHistoryController::class, 'indexEventHistory'])->name('admin.history.events');


        Route::post($routes['logout'], [AdminAuthController::class, 'logout']) -> name('admin.logout');
    });
